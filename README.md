...
|---|---|---|---|
| 1 | Quem são os clientes com assinatura ativa hoje e em que plano estão? | `JOIN` (2), `WHERE`, `ORDER BY` | [`01`](queries/01_clientes_ativos_e_seus_planos.sql) |
| 2 | Quantos assinantes ativos e quanto de MRR cada plano gera? | `JOIN`, `WHERE`, `GROUP BY`, `ORDER BY` | [`02`](queries/02_assinantes_e_mrr_por_plano.sql) |
| 3 | Qual canal de aquisição traz mais clientes e qual traz mais receita? | `JOIN` (2), `GROUP BY`, `ORDER BY` | [`03`](queries/03_aquisicao_por_canal.sql) |
| 4 | Quanto de receita foi efetivamente recebida em cada mês? | `WHERE`, `GROUP BY`, `ORDER BY` | [`04`](queries/04_receita_recebida_por_mes.sql) |
| 5 | Quem cancelou, de que plano saiu e quanto tempo ficou na base? | `JOIN` (2), `WHERE`, `ORDER BY` | [`05`](queries/05_clientes_que_cancelaram.sql) |
| 6 | Quantos cancelamentos por mês e quanto de MRR foi perdido? | `JOIN`, `WHERE`, `GROUP BY`, `ORDER BY` | [`06`](queries/06_cancelamentos_por_mes.sql) |
| 7 | Onde a cobrança falha mais: em qual plano e em qual meio de pagamento? | `JOIN` (2), `WHERE`, `GROUP BY` duplo | [`07`](queries/07_falha_de_cobranca_por_plano_e_metodo.sql) |
| 8 | Quais clientes ativos têm cobrança em aberto? | `JOIN` (3), `WHERE` composto, `GROUP BY` | [`08`](queries/08_clientes_ativos_com_pagamento_em_aberto.sql) |
| 9 | Qual o volume de tickets e a satisfação média por categoria? | `WHERE IS NOT NULL`, `GROUP BY`, `ORDER BY` | [`09`](queries/09_suporte_por_categoria.sql) |
| 10 | Quais clientes ativos avaliaram o suporte com nota baixa? | `JOIN` (3), `WHERE IN`, `GROUP BY`, `ORDER BY` duplo | [`10`](queries/10_clientes_ativos_em_risco.sql) |
